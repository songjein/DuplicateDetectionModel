...
class TextCNN(object):
    ...
    """
    def __init__(
      self, sequence_length, vocab_size, embedding_size, filter_sizes, num_filters, learning_rate):
        ...
        self.y_data = tf.placeholder(tf.int32, [None, 1], name='y')
        self.Y_one_hot = tf.one_hot(self.y_data, 2)  # one hot
        ...
        with tf.name_scope("output"):
            self.output = tf.contrib.layers.fully_connected(self.fc_merged_drop2, 2, activation_fn=None)
        ...
        with tf.name_scope("loss"):
            ...
            cost_i = tf.nn.softmax_cross_entropy_with_logits(logits=self.output, labels=self.Y_one_hot)
